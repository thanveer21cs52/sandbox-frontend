...
async function Banner2() {
    const data1 = await fetchData(
      `https://strapi-backend-dbhx.onrender.com/api/service?populate[servicebanner2][populate]=*`
    );
    console.log(data1.servicebanner2, "sd");
    const data = data1.servicebanner2;
...
>
  ...
  <svg
    ...
    className='h-12 w-12 text-pink-300 mb-4'
    fill="none"
    stroke="currentColor"
  >
    {/* Outer shield stroke */}
 ...
 <svg
  ...
  >
    ...
    return (
        <div className="flex flex-col-reverse lg:flex-row bg-bg  md:px-20 lg:px-10">
            ...
            <div className="flex  flex-col justify-start items-start  flex-1/10   gap-5 text-dark-base  ">
              ...
              <div className=" flex flex-col  py-18 px-4 lg:px-0 gap-3 lg:flex-4/10">
                <div className="flex flex-col md:flex-row  gap-3 m-0 md:ml-18">
                     {
                data?.list?.map((dt:any,i:number)=>{
                  if(i<2){
                    return  <Reusecard icon={icons[i]} iconclass="text-4xl" divclass={`${colorMap[dt.color]} ${i==0?'w-full md:max-w-2/5 ':'w-full md:w-full md:mt-5'} md:px-5   ` } maintext={dt.title} smalltext={dt.text} key={i}/>
                ...
                <div className="flex flex-col md:flex-row  gap-3 m-0 md:mr-18">
                       {
                 data?.list?.map((dt:any,i:number)=>{
                     if(i>1){
                    return  <Reusecard icon={icons[i]} iconclass="text-4xl" divclass={`${colorMap[dt.color]}  ${i==2?'w-full md:max-w-2/5 ':'w-full md:w-full md:mb-5'} md:px-5  ` } maintext={dt.title} smalltext={dt.text} key={i}/>
