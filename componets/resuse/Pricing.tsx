...
function Pricing(props:any) {
    const [monthly,setsetmonthly]=useState(true)
    const icons=[
         <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 512 412.9"
    className="w-14 h-12 text-black mb-3"
  >
    {/* Basket base */}
    <path
    ...
    {/* Circles (items in basket) */}
    <circle className="fill-blue-300" cx="178.1" cy="149.9" r="35.1" />
    <circle className="fill-blue-300" cx="333.9" cy="149.9" r="35.1" />

    {/* Basket outline and handles */}
    <path
  ...
  >
    ...
    {/* Building block / home body */}
    <path className="fill-blue-300" d="M171.8 325.7h136.1v171.9H171.8z" />
  ...
  <svg
    ...
    className="w-14 h-12 text-primary mb-3"
  >
    {/* Handle */}
    <path
      className="stroke-current"
    ...
    return (
        <div className="relative bg-bg min-w-full max-md:h-[1450px] md: min-h-[950px] lg:min-h-[70vh] flex justify-end items-end">
             ...
             <div className=" flex flex-col gap-6 justify-center items-center absolute -top-44 left-[4vw] w-11/12">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-full gap-7 sm:px-20 md:px-0">

                            {
                                ...
                                //     }
                                // ]
                                props.data.pricelist[0][monthly?'monthly':'yearly'].map((data: any, i: number) => {
...
{icons[i]}
                                        ...
                                        <div className="w-full">
                                            <div className=" text-gray-500 flex justify-center gap-1 text-xl">
                                                ...
                                                <span className="text-4xl">{data.money}</span>
                                                <span className="mt-3">/{monthly?'mo':'yr'}</span>
                                            </div>

                                        </div>
                                        <div className="flex flex-col gap-3 w-full items-center justify-center mx-2">
                                               {
                                            data.list.map((li:any,i:number)=>{
                                                return  <Ticklist name={li[0]} icon={li[1]=='checked'?"right":'wrong'} key={i} />
                                            })
                                        }
